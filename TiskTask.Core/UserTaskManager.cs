...
namespace TiskTask.Core
{
  public class UserTaskManager
  {
    ...
    #region Логика трекинга времени и управления статусами

    public UserTask? GetActiveTask(long userId)
    {
      return _context.UserTasks.FirstOrDefault(t => t.UserId == userId && t.Status == UserTaskStatus.InProgress);
    }
    ...
    public UserTask SwitchActiveTask(long userId, int taskId, DateTime? switchedAtUtc = null)
    {
      ...
      // Приостановка старой активной задачи
    ...
    public bool CompleteTask(long userId, int taskId, DateTime? completedAtUtc = null)
    {
      ...
      if (task == null || task.Status == UserTaskStatus.Completed)
        return false;
    ...
    {
      if (task == null || task.Status != UserTaskStatus.InProgress)
        return;
      ...
      {
        var interval = pauseMomentUtc - task.StartedAtUtc.Value;
        if (interval > TimeSpan.Zero)
          task.TimeSpent += interval;
      }
    }
